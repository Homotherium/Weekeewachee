...
void MyGLItem::printdiscLists()
{
    // Print Black List
    for (int b = 0; b < m_blackdiscs_list.size(); b++) {
        qDebug() << m_blackdiscs_list[b]->getDisc_Name() << m_blackdiscs_list[b]->getDisc_Color() << m_blackdiscs_list[b]->getHoldCoordinates();
    }
    // Print White List
    for (int w = 0; w < m_whitediscs_list.size(); w++) {
        qDebug() << m_whitediscs_list[w]->getDisc_Name() << m_whitediscs_list[w]->getDisc_Color() << m_whitediscs_list[w]->getHoldCoordinates();
    }
}
